    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('admin.deliveries.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition duration-200 shadow-sm">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <select onchange="updateDeliveryStatus({{ $delivery->id }}, this.value)" 
                                class="mt-2 px-4 py-2 text-sm rounded-lg font-semibold border border-gray-300 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-sm font-bold text-gray-900 mt-1">{{ $delivery->updated_at->format('F d, Y H:i') }}</p>
                    </div>
                </div>
                ...
                <div class="relative mt-4">
                    <div class="flex justify-between mb-2">
                        <div class="text-center flex-1">
                            <div class="text-xs font-semibold {{ $delivery->delivery_status == 'pending' ? 'text-yellow-600' : 'text-blue-600' }}">
                                Pending Dispatch
                            </div>
                        </div>
                        <div class="text-center flex-1">
                            <div class="text-xs font-semibold {{ $delivery->delivery_status == 'in_progress' ? 'text-blue-600' : ($delivery->delivery_status == 'delivered' ? 'text-blue-600' : 'text-gray-400') }}">
                                In Transit
                            </div>
                        </div>
                        <div class="text-center flex-1">
                            <div class="text-xs font-semibold {{ $delivery->delivery_status == 'delivered' ? 'text-green-600' : 'text-gray-400' }}">
                                Arrived / Delivered
                            </div>
                        </div>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                        @php
                            $progress = 0;
                            if ($delivery->delivery_status == 'pending') $progress = 33;
                            elseif ($delivery->delivery_status == 'in_progress') $progress = 66;
                            elseif ($delivery->delivery_status == 'delivered') $progress = 100;
                        @endphp
                        <div class="bg-blue-600 rounded-full h-2.5 transition-all duration-500" style="width: {{ $progress }}%"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Unit Purchase Price</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity Ordered</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        @if($delivery->order && $delivery->order->products)
                            @forelse($delivery->order->products as $product)
                                @php
                                    // Safeguard unit pricing values mapping against your pivot updates
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-12 w-12 bg-gray-100 border rounded-lg overflow-hidden flex items-center justify-center">
                                                @if($product->images && $product->images->count() > 0)
                                                    <img src="{{ Storage::url($product->images->first()->image_path) }}" class="h-full w-full object-cover">
                                                @else
            ...
            <!-- Invoice Invoice Summary Total footer -->
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end">
                <div class="text-right">
                    <span class="text-xs font-semibold uppercase text-gray-500 block">Total Cart Value Balance</span>
                    <span class="text-xl font-black text-gray-900">${{ number_format($calculatedRunningTotal, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="flex justify-between items-center border-b border-gray-100 pb-2">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ $delivery->order->created_at->format('F d, Y H:i') }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-100 pb-2">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ $delivery->order->delivery_name ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex justify-between items-center border-b border-gray-100 pb-2">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ $delivery->order->user->name ?? 'Deleted Profile Account' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-100 pb-2">
                        <span class="text-sm font-medium text-gray-500">Email Address:</span>
                        <a href="mailto:{{ $delivery->order->user->email ?? '#' }}" class="text-sm font-semibold text-blue-600 hover:underline">{{ $delivery->order->user->email ?? 'N/A' }}</a>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm font-medium text-gray-500">Primary Contact Phone:</span>
                        <span class="text-sm font-bold text-gray-800">{{ $delivery->order->user->ph_number ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden lg:col-span-2">
                ...
                <div class="p-6">
                    <div class="bg-gray-50 border rounded-lg p-4 font-mono text-sm text-gray-800 leading-relaxed shadow-inner">
        ...
        <div class="mt-6 flex justify-end gap-3">
            <button type="button" onclick="showTracking({{ $delivery->id }})" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg font-medium flex items-center gap-2 shadow-sm transition">

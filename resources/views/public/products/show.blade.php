...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex mb-6 text-sm">
            <a href="{{ url('/') }}" class="text-gray-500 hover:text-blue-600">Home</a>
            <span class="mx-2 text-gray-400">/</span>
            <a href="{{ route('public.products.index') }}" class="text-gray-500 hover:text-blue-600">Products</a>
            <span class="mx-2 text-gray-400">/</span>
            ...
            <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div class="space-y-6">
                        ...
                        <div class="flex items-center gap-3 mt-3">
                            @if($product->has_promotion)
                                <div>
                                    ...
                                    <p class="text-3xl font-bold text-red-600 mt-1">${{ number_format($product->promotion_price, 2) }}</p>
                                </div>
                            @else
                    ...
                    <div class="border-t border-gray-100 pt-4">
                        <h3 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Specifications</h3>
                ...
                <div class="border-t border-gray-100 pt-6 mt-6">
                    @if($product->status !== 'out_of_stock')
                        <div class="bg-blue-50 p-4 rounded-xl text-center">
                            <i class="fas fa-lock text-blue-600 mb-2 block"></i>
                            <p class="text-sm text-gray-700 mb-3">Please login to purchase this product</p>
                            <a href="{{ route('login') }}" class="inline-block w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-lg transition text-center">
                                Login to Order
                            </a>
                    ...
                    @else
                        <div class="bg-red-50 border border-red-200 p-4 rounded-xl text-center text-red-700 font-medium text-sm">
        ...
        <div class="mt-12">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition relative">
                    ...
                    @endif
                    <a href="{{ route('public.products.show', $related->id) }}" class="block bg-gray-50">
                    ...
                    <div class="p-3">
                        <h3 class="font-bold text-gray-800 text-xs truncate">{{ $related->name }}</h3>
                        @if($related->has_promotion)
                            <p class="text-xs text-gray-400 line-through">${{ number_format($related->original_price, 2) }}</p>
                            <p class="text-sm font-bold text-red-600">${{ number_format($related->promotion_price, 2) }}</p>
                        @else
                            <p class="text-sm font-bold text-blue-600">${{ number_format($related->price, 2) }}</p>
                        @endif
                    </div>
                </div>
                @endforeach

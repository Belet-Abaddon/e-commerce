...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
            ...
            @if(count($cart) > 0)
                <!-- Form submits directly to your session batch checkout pipeline -->
                <form method="POST" action="{{ route('user.cart.checkout') }}" enctype="multipart/form-data">
                    @csrf

                    <div class="p-6 space-y-6">
                        <!-- Product Items Manifest Summary List Table -->
                        <div class="bg-blue-50/30 border border-blue-100 rounded-xl overflow-hidden shadow-sm">
                            <div class="px-4 py-3 bg-blue-50/70 border-b border-blue-100 flex justify-between items-center">
                                ...
                                <span class="bg-blue-200 text-blue-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{{ count($cart) }} Distinct Models</span>
                            </div>
                                ...
                                @foreach($cart as $id => $item)
                                    <div class="p-4 flex items-center justify-between gap-4">
                                        ...
                                        <div class="flex items-center space-x-6 flex-shrink-0">
                                            ...
                                            <span class="text-sm font-black text-gray-900">${{ number_format($item['price'] * $item['qty'], 2) }}</span>
                        ...
                        <!-- Total Amount Banner Card Summary -->
                        <div class="bg-gray-50 p-4 border rounded-xl shadow-inner">
                            <div class="flex justify-between items-center">
                                ...
                                <span id="totalPrice" class="text-2xl font-black text-blue-600">
                        ...
                        <!-- Delivery Mode Options Selectors -->
                        <div>
                        ...
                        <!-- Delivery Courier Assigned Operations Matrix -->
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Assigned Delivery Courier Line *</label>
                            
                            <!-- Local Dispatch Couriers Grid -->
                            <div id="localCouriers" class="grid grid-cols-1 md:grid-cols-2 gap-3 animate-fade-in">
                                <label class="flex items-center p-3 border rounded-xl cursor-pointer hover:bg-gray-50 transition">
                                    <input type="radio" name="delivery_name" value="Royal Express" class="mr-3 text-blue-600" checked>
                                    <div>
                            ...
                            <!-- Global Cargo Freight Couriers Grid (Initially Hidden) -->
                            <div id="globalCouriers" class="grid grid-cols-1 md:grid-cols-2 gap-3 hidden animate-fade-in">
                                <label class="flex items-center p-3 border rounded-xl cursor-pointer hover:bg-gray-50 transition">
                        ...
                        <!-- Combined Consignee Identity Information -->
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Consignee Full Name *</label>
                            <input type="text" name="delivery_name_user" value="{{ Auth::user()->name ?? '' }}" disabled
                        ...
                        <!-- Financial Payment Gateways Settlement Grid System -->
                        <div>
                        ...
                        <!-- Dynamic Account Reference/Holder Details -->
                    ...
                    <!-- Actions Form Submissions Footer Area Bar -->
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-6 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700 transition shadow-sm flex items-center gap-2">
                            <i class="fas fa-lock text-xs"></i> Place Combined Order
                        </button>
                    </div>
                </form>
            @else
                <!-- Elegant Empty Cart State Blueprint Option Fallback -->
                <div class="text-center py-16 p-6">
                    ...
                    <a href="{{ route('user.products.index') }}" class="inline-flex bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm py-2 px-6 rounded-lg transition shadow-sm">
...
<script>
    // Toggle highlighted borders visually across active options sets
    ...
    // Capture delivery type switches safely 
    ...
    // Capture payment gateways switch triggers pipeline
    ...
    // File preview sequence logic trace block

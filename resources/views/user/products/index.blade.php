                ...
                <div class="lg:w-1/4 space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                        <div class="flex justify-between items-center mb-4 border-b pb-2">
                            ...
                            @if(request('brand_id'))
                                <button type="button" onclick="clearFilter('brand_id')"
                                    class="text-xs text-red-500 font-semibold hover:underline">Clear</button>
                            @endif
                        </div>
                        <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
                            @foreach($brands as $brand)
                                <label class="flex items-center cursor-pointer p-1 rounded hover:bg-gray-50 transition">
                                    <input type="radio" name="brand_filter" value="{{ $brand->id }}"
                                        class="brand-filter text-blue-600 focus:ring-blue-500 mr-3" {{ request('brand_id') == $brand->id ? 'checked' : '' }}>
                                    <span
                                        class="text-sm {{ request('brand_id') == $brand->id ? 'text-blue-600 font-bold' : 'text-gray-700' }}">{{ $brand->name }}</span>
                                    <span
                                        class="ml-auto text-xs text-gray-400 font-mono">({{ $brand->products_count ?? 0 }})</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                        <div class="flex justify-between items-center mb-4 border-b pb-2">
                            ...
                            @if(request('product_type_id'))
                                <button type="button" onclick="clearFilter('product_type_id')"
                                    class="text-xs text-red-500 font-semibold hover:underline">Clear</button>
                            @endif
                        </div>
                        <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
                            @foreach($productTypes as $type)
                                <label class="flex items-center cursor-pointer p-1 rounded hover:bg-gray-50 transition">
                                    <input type="radio" name="type_filter" value="{{ $type->id }}"
                                        class="type-filter text-blue-600 focus:ring-blue-500 mr-3" {{ request('product_type_id') == $type->id ? 'checked' : '' }}>
                                    <span
                                        class="text-sm {{ request('product_type_id') == $type->id ? 'text-blue-600 font-bold' : 'text-gray-700' }}">{{ $type->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                        ...
                        <div class="space-y-4">
                            <div class="flex gap-3">
                                <div class="flex-1">
                                    <label
                                        class="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">Min
                                        ($)</label>
                                    <input type="number" id="min_price" placeholder="{{ $minPrice }}"
                                        value="{{ request('min_price') }}"
                                        class="w-full text-sm px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                                </div>
                                <div class="flex-1">
                                    <label
                                        class="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">Max
                                        ($)</label>
                                    <input type="number" id="max_price" placeholder="{{ $maxPrice }}"
                                        value="{{ request('max_price') }}"
                                        class="w-full text-sm px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                                </div>
                            </div>
                            <button id="applyPriceBtn"
                                class="w-full bg-blue-600 text-white font-bold text-sm py-2 rounded-lg hover:bg-blue-700 transition shadow-sm">
                                Apply Price Range
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center cursor-pointer p-1 rounded hover:bg-gray-50">
                                <input type="radio" name="sort" value="newest"
                                    class="sort-option text-blue-600 focus:ring-blue-500 mr-3" {{ (request('sort') == 'created_at' || !request('sort')) ? 'checked' : '' }}>
                                <span class="text-sm text-gray-700">Newest First</span>
                            </label>
                            <label class="flex items-center cursor-pointer p-1 rounded hover:bg-gray-50">
                                <input type="radio" name="sort" value="price_asc"
                                    class="sort-option text-blue-600 focus:ring-blue-500 mr-3" {{ (request('sort') == 'price' && request('order') == 'asc') ? 'checked' : '' }}>
                                <span class="text-sm text-gray-700">Price: Low to High</span>
                            </label>
                            <label class="flex items-center cursor-pointer p-1 rounded hover:bg-gray-50">
                                <input type="radio" name="sort" value="price_desc"
                                    class="sort-option text-blue-600 focus:ring-blue-500 mr-3" {{ (request('sort') == 'price' && request('order') == 'desc') ? 'checked' : '' }}>
                                <span class="text-sm text-gray-700">Price: High to Low</span>
                            </label>
                            <label class="flex items-center cursor-pointer p-1 rounded hover:bg-gray-50">
                                <input type="radio" name="sort" value="name_asc"
                                    class="sort-option text-blue-600 focus:ring-blue-500 mr-3" {{ (request('sort') == 'name' && request('order') == 'asc') ? 'checked' : '' }}>
                ...
                <div class="lg:w-3/4">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-4 mb-6 border border-gray-100">
                        <form method="GET" action="{{ route('user.products.index') }}" class="flex gap-3">
                            ...
                            @if(request('brand_id')) <input type="hidden" name="brand_id" value="{{ request('brand_id') }}">
                            @endif
                            @if(request('product_type_id')) <input type="hidden" name="product_type_id"
                            value="{{ request('product_type_id') }}"> @endif
                            ...
                            @if(request()->anyFilled(['search', 'brand_id', 'product_type_id', 'min_price', 'max_price']))
                                <a href="{{ route('user.products.index') }}"
                                    class="bg-gray-500 text-white px-5 py-2 rounded-lg hover:bg-gray-600 flex items-center justify-center transition"
                    ...
                    @if(isset($mostOrdered) && $mostOrdered->count() > 0 && !request()->anyFilled(['search', 'brand_id', 'product_type_id']))
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                @foreach($mostOrdered as $item)
                                    <div
                                        class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition flex flex-col justify-between relative">
                                        ...
                                        <div class="p-3 flex-1 flex flex-col justify-between">
                                            <div>
                                                <span
                                                    class="text-[10px] uppercase font-extrabold text-gray-400 block tracking-wider">{{ $item->brand->name ?? 'HomeNest' }}</span>
                                                <h3 class="font-bold text-gray-800 text-xs mt-0.5 line-clamp-1">{{ $item->name }}
                                                </h3>
                                            </div>
                                            <div class="mt-2 pt-2 border-t border-gray-50 flex items-center justify-between">
                                                ...
                                                <a href="{{ route('user.products.show', $item->id) }}"
                                                    class="bg-blue-600 text-white px-2.5 py-1 text-xs font-bold rounded hover:bg-blue-700 transition shadow-sm">
                                                    View
                                                </a>
                                            </div>
                                        </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <p class="text-xs text-gray-400 font-medium font-mono">{{ $products->total() }} matches listed
                            </p>
                        </div>
